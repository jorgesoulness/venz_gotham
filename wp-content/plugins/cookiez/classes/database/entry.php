...
use Cookiez\Classes\Database\Exceptions\Missing_Table_Exception;
...
class Entry {
	 ...
	 * set
	 *
	 * Sets an entry data field value by the specified key
	 *
	 * @param string $name  The key of the field being set
	 * @param mixed  $value The new value for the field
	 *
	 ...
	 * Raises the hook [cookiez/db/class_name>/change]. If the /event/
	 * parameter is not null, then it also raises the hook [cookiez/db/class_name/event].
	 *
	...
	private function trigger_change( $data, ?string $event = null ): void {
		if ( $event ) {
			 ...
			 * @var false|int $data
			 */
			do_action( 'cookiez/db/' . $this->class_short_name() . '/' . $event, $this, $data );
		}
		 ...
		 * @var false|int $data
		 */
		do_action( 'cookiez/db/' . $this->class_short_name() . '/change', $this, $data );
	}
